...
- **Per-race +2 bonuses (Stage 2 rule tweak):** still +2 each for pole, fastest lap, most laps led, lowest incidents — but pole and fastest lap are now judged among LEAGUE drivers only (Stage 1 logic unchanged, handled by `stageNumber` in `calculateBonuses`):
  - **Fastest lap:** awarded to the fastest lap among humans, even if an AI ran quicker.
  - **Pole:** awarded to the HIGHEST HUMAN QUALIFIER (best/lowest start position among league drivers), not just whoever started P1 overall.
  - **Lowest incidents:** awarded among FINISHERS only — a driver who did not finish the race (DNF, `out_reason` other than "Running") is ineligible, and the bonus (with ties split) goes to the cleanest driver who actually finished. (Applies to `calculateBonuses` for all stages; Stage 1 data is stored and unaffected.)
